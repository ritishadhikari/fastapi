...
def test_read_one_authenticated(test_todo):
    response=client.get(url="/todo/1")
    assert response.status_code==status.HTTP_200_OK
    assert response.json()=={
        "title":"Learn to code!",
        "description":"Need to learn everyday!",
...
def test_read_one_authenticated_not_found(test_todo):
    response=client.get(url="/todo/999")
...
def test_create_todo(test_todo):
    ...
    response=client.post(url="/todo",json=request_data)
...
def test_update_todo(test_todo):
    ...
    # /todo_update/{todo_id}
    response=client.put(url="/todo_update/1",json=request_data)
...
def test_update_todo_not_found(test_todo):
    ...
    # /todo_update/{todo_id}
    response=client.put(url="/todo_update/999",json=request_data)
    print(response.json())
    assert response.status_code==status.HTTP_404_NOT_FOUND
    
def test_delete_todo(test_todo):
    response=client.delete(url="/todo/delete/1")
    assert response.status_code==status.HTTP_204_NO_CONTENT
...
def test_delete_todo_not_found(test_todo):
    response=client.delete(url="/todo/delete/999")
    assert response.status_code==status.HTTP_404_NOT_FOUND
